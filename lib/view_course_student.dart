...
class ViewCourseStudent extends StatefulWidget {
  const ViewCourseStudent({super.key, required this.courseID});
  final String courseID;

  @override
  State<ViewCourseStudent> createState() => _ViewCourseStudentState(courseID: this.courseID);
}

class _ViewCourseStudentState extends State<ViewCourseStudent> {
  _ViewCourseStudentState({required this.courseID});
                  ...
                  child: Column(
                    children: <Widget>[
                      ...
                      FormBuilderTextField(
                        autovalidateMode: AutovalidateMode.always,
                        name: 'teacher_name',
                        initialValue: jsonDecode(snapshot.data?.get('teachers')?[0])['name'],
                        decoration: const InputDecoration(
                      ...
                      ElevatedButton(
                        onPressed: () {
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                                builder: (context) =>
                                    ListStudentLectures(courseID:courseID)),
                          );
                        },
